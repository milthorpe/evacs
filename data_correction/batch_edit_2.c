...
static void copy_entry_down(      PGconn       *conn,
   ...
   if (from_entry_id <= 0) {
      fprintf(stderr,
              "ERROR: (copy_entry_down) Invalid value for "
                            ...
                            const unsigned int  electorate_code)
{
   ...
   fprintf(stderr, "\nEntered delete_duplicate_paper\n"
                     "==============================\n");
   fprintf(stderr, "    batch_number .. %u\n", batch_number);
   ...
   /* SIPL 2014-05-20 Free normalized name. Do it now, as there
      are multiple return statements further on. */
   free(electorate_name_normalized);
