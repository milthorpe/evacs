...
*/
{
	...
	char *electorate_name=resolve_electorate_name(conn,e_code);
	/* SIPL 2014-05-20 Support electorate names with spaces. */
	char *electorate_name_normalized=resolve_electorate_name_normalized(conn,e_code);
	char *active_flag;
	const char *tab         = (char *) "\t";
	...
	/* SIPL 2014-05-20 Free these strings. (Why wasn't electorate_name)
	   freed before?) */
...
*/
{
	...
	/* SIPL 2014-05-22 Support electorate names with spaces. */
	...
	electorate_name_normalized = malloc(strlen(electorate_name) + 1);
	...
	fprintf(stderr,
		...
		electorate_name_normalized);
	bwe = find_errors_in_batch(conn, batch, electorate_name_normalized);
...
*/
{
	...
	electorate_name_normalized = malloc(strlen(elec_name) + 1);
	normalize_electorate_name(electorate_name_normalized,
			     elec_name);
...
*/
{
	...
	electorate_name_normalized = malloc(strlen(get_voter_electorate()->name) + 1);
	normalize_electorate_name(electorate_name_normalized,
				  get_voter_electorate()->name);
...
*/
{
	...
	result = SQL_query(conn,
			   ...
			   "AND p.index <= b.size;",
			   electorate_name_normalized, electorate_name_normalized, 
			   batch_number,electorate_name_normalized);
	...
	/* SIPL 2014-05-20 Free normalized name. */
	free(electorate_name_normalized);
		...
		for (i=0; i< num_entries; i++) {
		  ...
		  fprintf(stderr, "    paper_index ... %d\n",   paper_index);
			...
			SQL_command(conn,
				    ...
				    electorate_name_normalized,entry_id
				);
		}
...
*/
{
	...
	/* SIPL 2014-05-20 Support electorate names with spaces. */
	char *electorate_name_normalized;
	electorate_name_normalized = malloc(strlen(electorate->name) + 1);
	normalize_electorate_name(electorate_name_normalized,
				  electorate->name);
...
*/
{
	...
	/* SIPL 2014-05-20 Commented out next line. Why was it here? */
	/* electorate_name = resolve_electorate_name(conn,electorate_code); */
...
*/
{
	...
	get_active_entries(conn, electorate_name_normalized, batch_number, paper_index,
	...
	if (entry_index == 0) {
		/* SIPL 2014-05-22 Free normalized name. */
		free(electorate_name_normalized);
		return;
	}
