...

{
	...
	/* get electorate name in order to access that Electorates
	   preference table */
	electorate_name_normalized = resolve_electorate_name_normalized(conn,
							     electorate_code);
	...
	num_papers = (unsigned int )SQL_singleton_int(conn,
				      ...
				      electorate_name_normalized, batch_number);
	...
	if (num_papers > 0)
	        get_papers_for_batch(conn, tmp, electorate_name_normalized);
...

{
	...
	char *electorate_name_normalized, *paper_table_name;
	struct predefined_batch *batch;
	...
	electorate_name_normalized = resolve_electorate_name_normalized(conn,
							     electorate_code);
	paper_table_name = sprintf_malloc("%s_paper",electorate_name_normalized);
...
/* SIPL 2015-05-20 electorate_name must already have been normalized. */
...
/* load the batch structure pointed to by batch with paper data*/
{
	...
	result =  SQL_query(conn,
			    ...
			    "WHERE batch_number= %u "
			    "ORDER BY index;", 
			    electorate_name_normalized, batch->b.batch_number);
	...
	for (i=0;i<num_rows;i++) {
		...
		batch->papers[i].entries = get_entries_for_paper(conn,
					    paper_id,electorate_name_normalized);
...

{
	...
	result =  SQL_query(conn,
			    ...
			    "WHERE paper_id= %u "
			    "ORDER BY index DESC;",
			    electorate_name_normalized,paper_id);
...
void append_entry(PGconn *conn,
     ...
     */
{	
	char *electorate_name_normalized, *entry_table_name, *paper_table_name;
        int  paper_id=-1;
        int temp;
	...
	/* get electorate name in order to access that Electorates'
	   paper and entry tables */
	paper_table_name = sprintf_malloc("%s_paper",
					  electorate_name_normalized);
	entry_table_name = sprintf_malloc("%s_entry",
					  electorate_name_normalized);
          ...
          */
          ...
          temp = (unsigned int)
            SQL_singleton_int(conn,
                              ...
                              "AND e.paper_id     = p.id "
                              "AND e.operator_id  = '%s';",
                              electorate_name_normalized, electorate_name_normalized,
	...
	update_active_entries(conn,
			     ...
			     (const char *)  electorate_name_normalized);
			       ...
			       int *active_entry2) 
{
	...
	result = SQL_query(conn,
			   ...
			   "WHERE index = %d "
			   "AND batch_number = %u;",
			   electorate_name_normalized, paper_index,batch_number
		);
...
int replace_active_entry(PGconn *conn,
	...
	char *paper_table_name = sprintf_malloc("%s_paper",electorate_name_normalized);
...
/* SIPL 2014-05-20 Same as resolve_electorate_name, but with the
   result normalized, i.e., with spaces, etc. replaced with underscores. */
char *resolve_electorate_name_normalized(PGconn *conn, 
			      unsigned int electorate_code)

{
	char *elec_name, *elec_name_normalized;
        elec_name = SQL_singleton(conn,
			     "SELECT name FROM electorate WHERE code = %u;",
			     electorate_code);
	elec_name_normalized = malloc(strlen(elec_name) + 1);
	normalize_electorate_name(elec_name_normalized, elec_name);
	free(elec_name);
	return elec_name_normalized;
}


/* SIPL 2014-05-21 Allow for electorate names with apostrophes. */
int resolve_electorate_code(PGconn *conn,
			    const char *electorate_name)
{
	/* SIPL 2014-05-21 Support electorate names with apostrophes. */
	char escaped_electorate_name[strlen(electorate_name) * 2 + 1];
	size_t escaped_electorate_name_length;
	int escape_error;

	/* SIPL 2014-05-20 Escape the electorate name so that it
	   can be passed to the SQL SELECT statement. */
	escaped_electorate_name_length = PQescapeStringConn (conn,
	   escaped_electorate_name, electorate_name, strlen(electorate_name),
	   &escape_error);
	if (escape_error != 0) {
		return 0;
	}
