...
static struct candidate *exclude_tiebreak(struct cand_list *candidates)
{
	...
	for (c = count-1; c >= 1; c--) {
		losers = any_candidates(candidates, &lowest_at_count,
					(void *)c);
		/* SIPL 2014-06-02 (No change was applied here; this
		   comment is for clarification only.)
		   At first sight, this code might appear to suffer from
		   the defect in surplus_tiebreak() fixed on 2012-02-06.
		   However, there is no problem here. lowest_at_count()
		   applies any_candidates() to totals of
		   the form "INT_MAX - n", which can never be zero.
		   Therefore, any_candidates() will not return an
		   empty list, and the use of losers->next will not fail.
		 */
		free_cand_list(candidates);
