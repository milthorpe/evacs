...
const char *OUTPUTROOT = "/tmp/evacs_export";
const char *OUTPUTDIR = "paper_ballots";
...
#define sprintf_malloc(x, ...) {\
    size_t needed = snprintf(NULL, 0, __VA_ARGS__) + 1;\
    x = malloc(needed);\
    snprintf(x, needed, __VA_ARGS__);\
}
/*
...
/*	char *sed_cmd;*/
	FILE *fh;
	char *path;
    sprintf_malloc(path, "%s/%s/%s",OUTPUTROOT,OUTPUTDIR,filename);
    //char *path="/tmp/evacs_export/paper_ballots/Groups.csv";
...
/* 		   DATABASE_NAME,SERVER_ADDRESS); */
  ...
  //sprintf_malloc(path, "/tmp/evacs_export/paper_ballots");
  sprintf_malloc(path, "%s/%s",OUTPUTROOT,OUTPUTDIR);
  ...
  sprintf_malloc(pv_filename, "%s/%s/%s",OUTPUTROOT,OUTPUTDIR,"Ballot_paper_versions.csv");
  ...
  for (i=0;electorates[i]!=NULL;i++)
  {
	  ...
	  /* open new electorate files */		  
		  sprintf_malloc(formal_filename, "%s/%s/%s_formal_papers.csv",
	  ...
	  fprintf(fh_formal,"\"Batch Number\",\"Paper Index\",\"Preference Number\",\"Party Code\",\"Candidate Code\",\"Candidate Screen Index\"\r\n"); 
		  sprintf_malloc(informal_filename, "%s/%s/%s_informal_papers.csv",
				 OUTPUTROOT,OUTPUTDIR,
				 electorate_name_normalized);
