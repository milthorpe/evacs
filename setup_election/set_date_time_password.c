...
// This program sets the password_hash_date_time value
// in the master-database table to the hash
...
int main(int argc, char**argv)
{
  ...
  printf("Note: only the first %d characters are "
    ...
    PASSWORD_LENGTH);
  printf("Enter a password to be used for changing the date/time:\n");
  ...
  SQL_command(conn,
	      "UPDATE master_data SET password_hash_date_time = '%s';",
	      password_encrypted);
  PQfinish(conn);
