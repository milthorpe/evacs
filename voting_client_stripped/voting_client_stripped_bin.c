...
int main(int argc, char *argv[])
{
   ...
   struct ballot_contents *bc ;
	...
	/* SIPL 2014-05-19 Removed the electorate name parameter,
	   reducing the number of expected parameters by one. */
	if (argc < 5 || argc > 6)
	{
	  fprintf(stderr, "\nStripped-down voting client used to preview ballot papers\n");
	  fprintf(stderr, "Usage: %s <electorate_code> <number_of_seats> <screen width> <screen height>\n", argv[0]);
	  fprintf(stderr, "The electorate_code & number_of_seats should be as specified in the electorates.txt file\n");
	  fprintf(stderr, "e.g. %s 1 5 1152 864\n", argv[0]);
	  return(-1);
	}
	/* SIPL 2014-05-19 Previously, the first parameter was the
	   electorate name, and so there was this line of code:
	     sprintf(e.name, argv[1]);
	   which is a buffer overrun.  So the electorate name
	   parameter has now been removed, and the other parameters
	   have been shuffled up by one place. */
	e.code=atoi(argv[1]);
	e.num_seats=atoi(argv[2]);
	...
	initialise_display(atoi(argv[3]), atoi(argv[4]), (argc == 6 ));
	init_session();
	show_ballot(&e, bc);
	sleep(60);
	return(0);
