...
bool initialise_display(int scrn_width, int scrn_height, bool is_upside_down)
{
	...
	screen_width = scrn_width;
	screen_height = scrn_height;
...
/* SIPL 2014-05-23 Increased from 20 to 25.
   NB Since it is a five-bit value, 31 is the maximum, which corresponds
   to white.
 */
#define HIGHLIGHT_GRAY 25
...
if (image->data == NULL)
fprintf(stderr, "Image Data is Null!!\n");
...
//fprintf(stderr, "get_image >%s< not in cache!\n", url);

	/* Otherwise, create new cache entry. */
	i = malloc(sizeof(*i) + strlen(url)+1);
	if (!i) return NULL;
	strcpy(i->name, url);
//fprintf(stderr, "strcpy %s ok\n", url);

	/* Ask server for image */
	buf = http_get(TEST_SERVER_ADDRESS, TEST_SERVER_PORT, url, &size);
	if (!buf) {
		fprintf(stderr, "http_get returned nothing!!!\n");
		free(i);
		return NULL;
	}

//fprintf(stderr, "got from server!\n");
...
//fprintf(stderr, "freed buf!\n");
	...
	i->next = image_cache;
	image_cache = i;
//fprintf(stderr, "get_image returning!\n");
	return &i->image;
}
